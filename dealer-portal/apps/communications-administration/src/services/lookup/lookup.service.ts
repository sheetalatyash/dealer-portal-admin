...
export class LookupService {
  ...
  /**
   * Retrieves a specific communication status entity by its name.
   *
   * @returns - An observable that emits the `CommunicationStatusEntity` matching the provided name, or `undefined` if not found.
   *
   * @example
   * // Get a status by name
   * getStatusByName$(CommunicationStatus.Active).subscribe(statusEntity => {
   *   console.log('Status Entity:', statusEntity);
   * });
   * @param statusType
   */
  public getStatusByName$(statusType: CommunicationStatusType): Observable<CommunicationStatus | undefined> {
    return this.getStatuses$().pipe(
      map((statuses: CommunicationStatus[]): CommunicationStatus | undefined => statuses.find((status: CommunicationStatus): boolean => status?.name === statusType))
    );
  }
